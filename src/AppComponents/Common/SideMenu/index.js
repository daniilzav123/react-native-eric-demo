...
export class SideMenu extends React.Component {
	...
	constructor(props, context) {
		super(props, context);
		const initialMenuPositionMultiplier = props.menuPosition === "right"
			? -1
			: 1;
		...
		this.state = {
			...
			left: new Animated.Value(
				props.isOpen
					? OPEN_MENU_OFFSET * initialMenuPositionMultiplier
	...
	componentWillMount() {
		this.responder = PanResponder.create({
			onStartShouldSetResponderCapture: this.props.onStartShouldSetResponderCapture.bind(
				this
			),
	...
	handlePanResponderEnd(e, gestureState) {
		...
		const offsetLeft =
			this.menuPositionMultiplier() * (left.__getValue() + gestureState.dx);
